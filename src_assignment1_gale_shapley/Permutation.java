...
public class Permutation {
    ...
    /**
     * Initialize a permutation which will select pairings of students to spots. There should be at
     * most p pairings. No pairing should be used more than once. No student should be used more than
     * once.
     *
     * @param n Number of students.
     * @param k Number of spots available.
     */
    ...
    private Matching convertPairingToMatching(Matching data, int[] pairing) {
        ...
        int m = data.getInternshipCount();
        int n = data.getStudentCount();
        ArrayList<Integer> internship_slots = data.getInternshipSlots();
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < internship_slots.get(i); j++) {
    ...
    /**
     * Select the next pairing in "ascending order." Each student can be assigned to at most one spot,
     * and each spot can be assigned to at most one student, and there are more students than there
     ...
     *
     * <p>Values for the students range from 0 to n-1, so let the counting system be a base-n counting
     ...
     *
     * <p>Keep track of the number of instances of a given student's number are present in the
     * pairing. A pairing is only valid if each student is paired with a position at most 1 time.
     */
