...
public class Main {
    ...
    public static void test(){
        ...
        if(encode && decode) {
            ...
            System.out.println("\nThe encoded message for " + message + " is:");
            String encodedMessage = huffman.encode(message);
            System.out.println(encodedMessage);
            System.out.println("\nThe decoded message for " + encodedMessage + " is:");
            time2 = System.nanoTime();
            System.out.println(huffman.decode(encodedMessage));
            System.out.println("Input string length is: " + message.length() + "*8 = " + 8*message.length() + " EncodedMessage length is: " + encodedMessage.length());
        }
        else if(encode){
            System.out.println("The encoded message for " + message + " is:");
            System.out.println(huffman.encode(message));
        }
        else{
            System.out.println("The decoded message for " + message + " is:");
            System.out.println(huffman.decode(message));
        }
        long endTime = System.nanoTime();
		System.out.println("Execution time in nanoseconds  : " + (endTime - startTime) + " first:" + (time1 - startTime) + " second:" + (time2 - time1) + " third:" + (endTime - time2));
    }
}
